...
协议版本是交易校验字段；npm、Go 和 Rust 版本是各自包的发布版本。Go tag 为 `v` 加 Go 版本，Rust tag 为 `rust-v` 加 Rust 版本。发布验证会检查 Go proxy 的大小写转义 module path，并用 Rust crate checksum 与 `.cargo_vcs_info.json` 证明产物来自当前 commit。
