...
var fields = []string{"LockHex", "FundingHex", "StateHex", "BuyerSignatureHex", "SellerSignatureHex", "FinalHex"}
...
func main() {
	...
	goOutput, err := run(root, "go", "run", filepath.Join("examples", "two_party_pool_compare", "go_runner", "main.go"))
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
	tsOutput, err := run(root, "npx", "tsx", filepath.Join("examples", "two_party_pool_compare", "ts_runner.ts"))
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
